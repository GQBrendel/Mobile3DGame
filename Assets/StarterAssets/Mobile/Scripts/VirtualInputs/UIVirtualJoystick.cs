...
{  
    ...
    public void OnPointerDown(PointerEventData eventData)
    {
        if (containerRect)
        {
            ...
            RectTransformUtility.ScreenPointToLocalPointInRectangle(screenRect, eventData.position, eventData.pressEventCamera, out Vector2 position);

            position = ApplySizeDelta(position);

            Vector2 clampedPosition = ClampValuesToMagnitude(position);

            UpdateConteinRectPosition(eventData.pressPosition);
        }
    ...
    private void UpdateConteinRectPosition(Vector2 newPosition)
    {
